...
class IngredienteReceitaTest {
    ...
    void shouldCreate() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(post("/api/receitas/{receitaId}/ingredientes", receitaId)
    ...
    @Test
    void shouldReturnIngredienteWhenFindByID() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(get("/api/receitas/{receitaId}/ingredientes/{ingredienteId}",
                receitaId, ingredienteId))
                .andExpect(status().isOk())
                .andReturn();

        IngredienteDTO ingredienteDTO = objectMapper.readValue(mvcResult.getResponse().getContentAsByteArray(),
    ...
    @Test
    void shouldReturnAllIngredientesFromReceitaID() throws Exception {
        final Long receitaId = 1L;
        final Long ingredienteId = 20L;
        ...
        when(ingredienteService.getAll(receitaId)).thenReturn(List.of(IngredienteMapper.toDTO(ingrediente),
                IngredienteMapper.toDTO(ingrediente)));

        MvcResult mvcResult = mockMvc.perform(get("/api/receitas/{receitaId}/ingredientes", receitaId))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void shouldUpdate() throws Exception {
        ...
        mockMvc.perform(put("/api/receitas/{receitaId}/ingredientes/{ingredienteId}", recitaId, ingredienteId)
                .contentType(APPLICATION_JSON_VALUE)
    ...
    void shouldDelete() throws Exception {
        ...
        mockMvc.perform(delete("/api/receitas/{receitaId}/ingredientes/{ingredienteId}", recitaId, ingredienteId)
                .contentType(APPLICATION_JSON_VALUE))
                .andExpect(status().isOk());
